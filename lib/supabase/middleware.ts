 ...
 * bounced away from /login. Runs in the Edge middleware.
 */
export async function updateSession(request: NextRequest) {
  let supabaseResponse = NextResponse.next({ request });

  // Before the keys are pasted in, let every route through so the site
  // still deploys and runs. Auth simply turns on once env vars are set.
